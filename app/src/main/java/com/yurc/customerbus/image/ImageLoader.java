...
public class ImageLoader {
    ...
    private void initBackThread() {
        ...
        mPoolThread = new Thread() {
            ...
            public void run() {
                Looper.prepare();
                LogUtil.v("initBackThread", "initBackThread");
                mPoolThreadHandler = new Handler() {
                    @Override
                    public void handleMessage(Message msg) {
                        // 线程池去取出一个任务进行执行
                        mThreadPool.execute(getTask());
                        LogUtil.v("mPoolThreadHandler", "mPoolThreadHandler");
                        try {
                ...
                if (isFromNet) {
                    ...
                    if (file.exists())// 如果在缓存文件中发现
                    {
                        ...
                        if (isDiskCacheEnable) {
                            boolean downloadState = DownloadImgUtils.downloadByUrl(path,
                                    file);
                            // 如果下载成功
                            if (downloadState) {
                               /* Log.e(TAG,
